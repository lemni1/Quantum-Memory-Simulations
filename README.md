...
This repository hosts the XMDS2 code for simulation gradient echo memory and cross phase modulation performed on gradient echo memory.  Included with the base XMDS2 code are jupyter notebooks that make it convenient for executing the code and visualizing the output.  The most reliable way run these simulations is via the docker image with xmds and the python packages installed.  
